# adapted from https://github.com/IBM/docAMR/blob/main/amr_io.py
...
def get_simple_graph(graph):
    ...
    for x in graph.edges():
        ...
        if (
            ...
            # block, since we do not need it anymore
...
pronouns = ['i', 'we', 'you', 'he', 'she', 'that', 'this', 'these', 'those', 'it', 'they', 'all', 'another', 'anybody',
            'anyone', 'anything', 'any', 'both', 'each', 'either', 'everybody', 'everyone', 'everything', 'neither',
            'nobody', 'none', 'nothing', 'one', 'few', 'most', 'many', 'other', 'several', 'some', 'somebody',
            'someone', 'something', 'such', 'whatever', 'whichever', 'whoever', 'whomever']
# personal_pronouns = ['i', 'we', 'you', 'he', 'she', 'they', 'one', 'someone', 'self']

# NE types ordered from specific to general
NETypes = ['medical-condition', 'disease', 'taxon', 'species', 'cell-line', 'cell', 'dna-sequence', 'gene', 'pathway',
           'nucleic-acid', 'enzyme', 'macro-molecular-complex', 'amino-acid', 'protein-segment', 'protein-family',
           'protein', 'small-molecule', 'molecular-physical-entity', 'program', 'variable', 'writing-script',
           'food-dish', 'musical-note', 'music-key', 'treaty', 'court-decision', 'law', 'award', 'natural-object',
           'journal', 'magazine', 'newspaper', 'book', 'publication', 'broadcast-program', 'show', 'music', 'picture',
           'work-of-art', 'car-make', 'spaceship', 'aircraft-type', 'aircraft', 'ship', 'vehicle', 'product',
           'festival', 'game', 'conference', 'war', 'earthquake', 'natural-disaster', 'incident', 'event',
           'amusement-park', 'zoo', 'park', 'market', 'sports-facility', 'worship-place', 'hotel', 'palace', 'museum',
           'theater', 'building', 'canal', 'railway-line', 'road', 'bridge', 'tunnel', 'port', 'station', 'airport',
           'facility', 'constellation', 'star', 'planet', 'moon', 'forest', 'desert', 'island', 'canyon', 'valley',
           'volcano', 'mountain', 'peninsula', 'canal', 'strait', 'bay', 'gulf', 'river', 'lake', 'sea', 'ocean',
           'continent', 'world-region', 'country-region', 'local-region', 'country', 'territory', 'province', 'state',
           'county', 'city-district', 'city', 'location', 'league', 'team', 'research-institute', 'university',
           'school', 'market-sector', 'political-party', 'criminal-organization', 'military', 'government-organization',
           'company', 'organization', 'political-movement', 'religious-group', 'regional-group', 'ethnic-group',
           'nationality', 'language', 'animal', 'family', 'person', 'thing']
...
class AMR():
    ...
    def from_penman(cls, penman_text, tokenize=False):
        ...
        return cls(tokens, nodes, edges, graph.top, penman=graph, alignments=alignments, sid=sid, nvars=nvars,
    ...
    def check_connectivity(self):
        ...
        for e in self.edges:
            if e[0] not in self.nodes:
                print(e)
                print("Bad edge")
            if e[1] not in ATTR_EDGES and e[2] not in self.nodes:
                print(e)
                print("Bad edge")
            ...
            if nid in self.nvars and self.nvars[nid] is not None:
                ...
                if tup not in all_tuples:
                    ...
                    if self.alignments and nid in self.alignments and self.alignments[nid] is not None:
                        epidata[tup] = [surface.Alignment(
                            indices=self.alignments[nid] if len(self.alignments[nid]) < 3 else [self.alignments[nid][0],
                                                                                                self.alignments[nid][
                                                                                                    -1]])]

        for e in self.edges:
            if e[1] in ATTR_EDGES:
                all_tuples.append(e)
                continue
            ...
            if self.nvars[e[2]] is None:
                ...
                if tup not in all_tuples:
                    ...
                    if self.alignments and e[2] in self.alignments and self.alignments[e[2]] is not None:
                        epidata[tup] = [surface.Alignment(
                            indices=self.alignments[e[2]] if len(self.alignments[e[2]]) < 3 else [
                                self.alignments[e[2]][0], self.alignments[e[2]][-1]])]
            else:
    ...
    def add_attr_edges(self, edges):
        for (s, l, t) in edges:
            if s in self.nvars2nodes:
                this_node = self.nvars2nodes[s]
...
if __name__ == '__main__':
    penman_str = """
    (d / document
       :snt1 (s1.c / contrast-01
               :ARG1 (s1.k / know-01
                           :ARG0 (pro / interlocutor-entity
                                      :domain pro1)
                           :ARG1 (s1.t3 / truth-value
                                        :polarity-of (s1.r / read-01
                                                           :ARG0 (s1.a / anyone)
                                                           :ARG1 (s1.t / this)))
                           :polarity "-")
               :ARG2 (s1.a2 / and
    ...
    amr_obj = AMR.from_penman(penman_str)
    nid = amr_obj.add_node("you", "s1.y")
    print(nid)
    amr_obj.add_edges([("s1.c", ":ARG0", nid)])
    print(amr_obj)